...
- [ ] Element Desktopで **プロフィール画像 → All Settings → Sessions** を開く
...
2. **検証済み端末はないが、過去を読めるElement Desktopがある** → [2B](#2b-検証済み端末がないが過去を読める)
...
リセットの前に、読めるElement Desktopが持つ復号鍵を退避します。

- [ ] 読めるElement Desktopをすべて起動したままにする
...
export完了後に限り、Element Desktopで次を実行します。
...
- [ ] 過去の暗号化履歴を失う可能性を理解する
- [ ] Element DesktopからデジタルIDをリセットする
- [ ] 新しいリカバリーキーを発行して保存する
- [ ] Secure Backupを有効にする
- [ ] 使用するすべての端末を検証する
